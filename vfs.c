...
//#define READ_SECTOR(x)       ((Drive.fpBlockRead)(Drive.ActSectorinBuffer=x,(INT16U*)((INT32U)Drive.pSectorBuffer),1))
//#define WRITE_SECTOR(x)      ((Drive.fpBlockWrite)(x,(INT16U*)((INT32U)Drive.pSectorBuffer),1))
//#define WRITE_SECTORS(x,d,n) ((Drive.fpBlockWrite)(x,(INT16U*)((INT32U) d),n))
//#define READ_SECTORS(x,d,n)  ((Drive.fpBlockRead)(x,(INT16U*)((INT32U) d),n))
//#define FLUSH_SECTOR         ((Drive.fpBlockWrite)(Drive.ActSectorinBuffer,(INT16U*)((INT32U)Drive.pSectorBuffer),1))
                ...
                for(ix = 0; ix < (Drive.V_FAT.SectorSize / sizeof(HD_VFS_INODE_DESC));ix++)
                  {
                    if(pInode->status == INODE_FREE){
                       ...
                       return err;
                       }else if(pInode->status == INODE_BUSSY){
                          file++;
                          pInode++;
                       }else{
                       printf("\nOut of file entry's!!!!!");
                        ...
                        if(!((HD_VFS_MARK_INFO*)&Drive.pSectorBuffer[((ix<<6)&0x1ff)])->type)
                          {
                            ...
                            if(!strlen((const char*)pInfo->markname)){
                                 ...
                                 printf((const char*)pInfo->markname);
                                }
                            pInfo->Idx = ix;
...
FAT_ERROR VFS_Mount(INT32U SecLba)
{
   ...
   VFS_Enter();
      ...
      if(tmp != 0xAA55){
         printf("\n partitiontable not aktive!!!! 0x%x",tmp);
         return FAT_ERROR_GEN;
        }
      for(i = 0; i < 4;i++)
         {
           ...
           if(!VFSHandler[i].stat)
            {
                ...
                while(is--)
                         ...
                         for(ix = 0; ix < (Drive.V_FAT.SectorSize / sizeof(HD_VFS_INODE_DESC));ix++)
                            {
                                ...
                                if(pInode[ix].status == INODE_BUSSY)
                                  {
                  ...
                  if((flag & O_OPEN_MASK) == O_OPEN)
                    {
                        if(pInode->status == INODE_BUSSY)
                          {
                             printf("\nopen file OK "); printf((const char*)path); printf(" Idx ->0x%x",i);
                 ...
                 //DEB_PR_ERROR("\n handler bussy ->")DEB_PR_INFO((const char*)VFSHandler[i].Inode.EntryName)

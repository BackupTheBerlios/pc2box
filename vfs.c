...
static FAT_ERROR hd_vfs_resetFileMarkList(INT16U idx)
{
    ...
    if(!Drive.V_FAT.infoentryLBA) {
        printf(" mark not supported!!!\n");
        return FAT_DEVICENOTSUPPORTED;
    }
    printf("  Reset MarkListInfo 0x%04x\n",idx);
    // byte by entry
...
static FAT_ERROR hd_vfs_flushFile(HD_VFS_HANDLER *pfile)
{
    ...
    printf("Flush File_Head\n");
    //! store file_head
    if (!READ_SECTOR(pfile->fileinfosector))
        return FAT_RW_ERROR;
    printf(" Head_LBA 0x%x\n",pfile->fileinfosector);
        ...
        if(pInode) {
            for(ix = 0;ix < (Drive.V_FAT.SectorSize / sizeof(HD_VFS_INODE_DESC));ix++) {
                ...
                if(stat == INODE_BUSY) {
                    ...
                    printf("%04d ",filenumber-1);
                    printf(" bmap lba 0x%x",pInode[ix].FileBitmapSector);
                    printf(" %-32s",pInode[ix].EntryName);
                    //Maindebug(out);
                    printf(" cs 0x%-4x",pInode[ix].sizecluster);
                    printf(" ls 0x%-8x\n",pInode[ix].sizeinlastcluster);
                } else if(stat == INODE_FREE) {
                    //printf("\n%d ",filenumber-1);
                    //printf("\n-- entry free --");
                } else if(stat == INODE_EOI) {
                    printf("-- EOI --\n");
                    return FAT_OK;
                } else {
                    printf("Inode ERROR!! 0x%x\n",stat);
...
static FAT_ERROR hd_vfs_Seek(HD_VFS_HANDLER *file,INT32S SkipNbyte)
{
    INT32S          NCluster;
    INT32U          Rembyte;
    FAT_ERROR       err = FAT_OK;
...
FAT_ERROR VFS_SetVFSRecordInfo(HD_VFS_MARK_INFO *pInfo,INT8U flLinkenable)
{
    ...
    if(!err){
        ...
        if (!FLUSH_SECTOR)
            return FAT_RW_ERROR;
    }else{printf("Set RecordMark SeekERROR!!!\n");}
    VFS_Exit();
    return err;
}
...
FAT_ERROR VFS_SetMarkInfoIDX(INT16U fileidx,INT16U markidx,HD_VFS_MARK_INFO *pInfo)
{
    ...
    if(!Drive.V_FAT.infoentryLBA){
        printf(" mark not supported!!!\n");
        return FAT_DEVICENOTSUPPORTED;
    } else if(Drive.V_FAT.numberoffilesinFS < fileidx) {
        printf(" fileIDX not supported 0x%x\n",fileidx);
        return FAT_DEVICENOTSUPPORTED;
    } else if(Drive.V_FAT.marksbyentry < markidx) {
        printf(" fileIDX not supported!!!\n");
        return FAT_DEVICENOTSUPPORTED;
    }
            ...
            if(!((HD_VFS_MARK_INFO*)&Drive.pSectorBuffer[((ix<<6)&0x1ff)])->type) {
                ...
                *(HD_VFS_MARK_INFO*)&Drive.pSectorBuffer[((ix<<6)&0x1ff)] = *pInfo;
                if (!FLUSH_SECTOR)
                    return FAT_RW_ERROR;
                break;
            } else if(((HD_VFS_MARK_INFO*)&Drive.pSectorBuffer[((ix<<6)&0x1ff)])->ActRecordNbr > pInfo->ActRecordNbr) {
                printf("Set Mark to Pos ->0x%x\n",ix);
                ...
                *(HD_VFS_MARK_INFO*)&Drive.pSectorBuffer[((ix<<6)&0x1ff)] = *pInfo;
                //! store LBA next time
                State = 1;
            } else if(((HD_VFS_MARK_INFO*)&Drive.pSectorBuffer[((ix<<6)&0x1ff)])->ActRecordNbr == pInfo->ActRecordNbr) { //set in same Block? do nothing!!!
                printf(" do not set Mark in same Block !!!\n");
                VFS_Exit();
                return FAT_ERROR_GEN;
            } else if(((HD_VFS_MARK_INFO*)&Drive.pSectorBuffer[((ix<<6)&0x1ff)])->type == MARK_FEND) {
                printf(" END !!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
                VFS_Exit();
                return FAT_ERROR_GEN;
            }
        } else if(State == 1) { //! resort until END
            //! save tmp
...
FAT_ERROR VFS_Mount(INT32U SecLba)
{
    ...
    for(i = 0; i < 4;i++) {
        printf(" test P->0x%x type 0x%x\n",i,Drive.pSectorBuffer[(0x1be + (i*0x10)) + 4]);
    ...
    printf(" mount VFS lba->0x%08x\n",SecLba);
    if (!READ_SECTOR(SecLba))
        return FAT_RW_ERROR;
    Drive.V_FAT = *(VFS_Table*)(Drive.pSectorBuffer);

    printf(" FreeBitmapLba ->0x%x\n" ,Drive.V_FAT.freebitmapsec     );
    printf(" BitmapbyteSize->0x%x\n" ,Drive.V_FAT.freelistsizeinbyte);
    printf(" RootEntries   ->0x%x\n" ,Drive.V_FAT.rootentrycounter  );
    printf(" RootEntSect   ->0x%x\n" ,Drive.V_FAT.rootentryLbaSect  );
    printf(" freeInodes    ->0x%x\n" ,Drive.V_FAT.freecounter       );
    printf("---VFS info system------>>>\n");
    printf(" LBA        ->0x%x\n"    ,Drive.V_FAT.infoentryLBA      );
    printf(" Entries    ->0x%x\n"    ,Drive.V_FAT.infoentrycounter  );
    printf(" LBASize    ->0x%x\n"    ,Drive.V_FAT.infosize          );
...
INT32U VFS_PutNByte(HD_VFS_HANDLER **pfile,INT8U *pData,INT32U Size,FAT_ERROR *err)
{
    ...
    if(*pfile == 0) {
        printf(" sorry invalid handler for this file\n");
        *err = FAT_ERROR_GEN;
        return 0;
    }
...
INT32U  VFS_GetNByte(HD_VFS_HANDLER **pfile,INT8U *pData,INT32U Size,FAT_ERROR *err)
{
    ...
    *err    = FAT_OK;
    ...
    if(file->ReadActCluster == file->lastCluster) {
        if((file->ReadClusterByteOffset + Size) > file->Inode.sizeinlastcluster) {
            printf("LINODE!!!\n");
            *err = FAT_EOC;
            VFS_Exit();
            return (Size - size);
        ...
        if(!VFSHandler[i].stat) {
            ...
            if (flag == O_RDWR) {
                if(pInode->status == INODE_BUSY) {
                    printf("open file OK "); printf("%s",(const char*)path); printf(" Idx ->0x%x\n",i);
                    ...
                    //! allocate buffer
                    printf(" allocate buffer for bitmap ->");
                    VFSHandler[i].stat              = 1;
                    (*pfile)                        = &VFSHandler[i];

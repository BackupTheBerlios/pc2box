...
void MyForm::updateFileListWidget(){
     ...
     sprintf((char*)Str,"%d",(int)FileCounter);

     itemCheckBox->setText((char*)Str);
     ...
     Flen   = Flen>>1;
     sprintf((char*)Str,"%d Kbyte",(int)Flen);
     item->setText( 2,(const char*)Str);

     Str[0] = 0x00;
     sprintf((char*)Str,"%d:%02d",ActVfsHandler.Inode.LastRecTime/60,ActVfsHandler.Inode.LastRecTime%60);
...
Disk_Thread::Disk_Thread(THRED_Params *pParams){
...
void Disk_Thread::UpdateFileListWidget(void){
     ...
     *this->pFileCounter =0;
     for(idx = 0;idx < VFS_ROOT_ENTRYS;idx++){
     ...
     if(!PCFile->open(QIODevice::ReadOnly)){
         ...
         if(strcmp((char*)pPC_Header->ver,VFS_PC_ACTVERSION)){
            ...
            if(CreateVFSEntry((char*)ActPc2BoxFile->EntryName)){
               ...
               HD_VFS_PutEventIngobyFileIDX(0,VFSHandler->EntryIDX,pPC_Header->Epg);
              }
           }
         free(pPC_Header);
       }
